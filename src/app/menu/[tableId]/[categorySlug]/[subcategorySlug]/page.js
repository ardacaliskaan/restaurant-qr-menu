  ...
  const loadData = async () => {
    try {
      ...
      if (categoriesData.success) {
        ...
        // Alt kategoriye ait ürünleri filtrele
        if (subcategory) {
          const filtered = (menuData.items || []).filter(item => 
            item.subcategoryId === subcategory.id && item.available !== false
          )
          
          // Eğer subcategoryId ile ürün bulunamadıysa, categoryId ile filtrele
          if (filtered.length === 0) {
            const parentCategory = allCategories.find(cat => cat.slug === categorySlug)
            if (parentCategory) {
              const categoryFiltered = (menuData.items || []).filter(item => 
                item.categoryId === parentCategory.id && item.available !== false
              )
